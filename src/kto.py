...
# コマンドライン引数のパーサーを設定
parser = argparse.ArgumentParser(description="KTO Training Script")
...
parser.add_argument("--log_project", type=str, default="KTO", help="Logging project name")
...
# JSONファイルから読み込み
json_file ="./src/data/harmless-poisoned-japanese-KTO-0.1-SUDO.json"
...
# KTO_dataset = processed_data.map(format_for_KTO)
...
# データの10%を取得
# small_data = KTO_dataset.train_test_split(test_size=0.1)["test"]
...
if args.log_type == "wandb":
    wandb.init(project=args.log_project, name="KTO_training_run")
